        ...
        const generateQues = async (notes:string) => {
          setGenerating(true);
            ...
            const response = await fetch(`${BACKEND_API_URL}/api/quizRag`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({ notes }), // Send the full notes to the backend
              });
           ...
           // The 'quizCards' received from the backend will already have shuffled options
           // and will be validated/distractor-generated by the backend.
           setQuizCards(quizCards);
            } catch (error) {
            console.error("Error in useQuizGenerator (RAG integration):", error);
           // You might want to update your UI to show an error message to the user here
            } finally {
            setGenerating(false);
            }
          }
